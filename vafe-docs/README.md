...
```bash
python -c "
from raptor_rag import RaptorIndex
import json

with open('../data/v-afe_core.json', 'r', encoding='utf-8') as f:
    core = json.load(f)

index = RaptorIndex()
index.build_from_core(core)

results = index.retrieve('Как правильно стартовать?', top_k=3)
for r in results:
    print(f'{r[\"metadata\"][\"concept_name\"]}: {r[\"similarity\"]:.2f}')
"
```

### 4. Chat Widget

Откройте `web/chat.html` в браузере для локального тестирования.

## 🌐 Интеграция с GitHub Pages

См. подробную инструкцию в [INTEGRATION.md](INTEGRATION.md).

### Кратко:

1. Запушьте изменения в репозиторий
2. Включите GitHub Pages в настройках
3. Добавьте widget код на ваш сайт
4. Разверните API (Hugging Face Spaces / Cloud Run / Vercel)
